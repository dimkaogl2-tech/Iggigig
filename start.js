...
const PORT = 49474 // Перепроверь порт на сайте Aternos, если сервер перезапускался!
...
// Максимальный список источников, отдающих тысячи прокси без лимитов
const PROXY_SOURCES = [
...
async function loadProxies() {
  console.log('⏳ Выкачивание огромной базы SOCKS5 прокси (1500+ адресов)...')
  let combinedProxies = []
  ...
  console.log(`📡 Глобальный пул собран. Всего на проверку отправлено: ${RAW_PROXIES.length} шт.`)
}
...
function checkProxy(proxyStr) {
  return new Promise((resolve) => {
    ...
    if (isNaN(port) || port <= 0 || port > 65535) return resolve(false)

    const socket = new net.Socket()
    socket.setTimeout(4500) // Увеличили таймаут чекера, чтобы поймать больше живых IP
...
async function filterProxies() {
  console.log('⚡ Пачечная проверка базы (по 60 сокетов одновременно)...');
  const BATCH_SIZE = 60 
  for (let i = 0; i < RAW_PROXIES.length; i += BATCH_SIZE) {
    ...
    // Как только нашли 40 живых прокси — запускаем ботов, чтобы долго не ждать
    if (VALID_PROXIES.length >= 40) break
  }
  console.log(`🔥 Проверка завершена! Найдено гарантированно ЖИВЫХ прокси: ${VALID_PROXIES.length}`)
}
    ...
    bot.on('spawn', () => {
      if (!spamTimer) {
        spamTimer = setInterval(() => {
          ...
          console.log(`✉ Бот [${botName}] успешно зашел на сервер и спамит!`)
        }, 5000)
      }
    })
...
async function main() {
  ...
  if (RAW_PROXIES.length === 0) {
    console.log('❌ Не удалось выкачать базу. Перезапустите скрипт.')
    return
  }
  
  await filterProxies()
  
  if (VALID_PROXIES.length === 0) {
    console.log('❌ Живых прокси не найдено. Защита блокирует запросы. Попробуйте еще раз.')
    return
  }

  console.log(`🤖 Пул готов (${VALID_PROXIES.length} шт). Запуск ${BOT_COUNT} ботов из облака...`)
