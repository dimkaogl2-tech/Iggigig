...
const PORT = 49474 // Убедитесь, что этот порт совпадает с портом на сайте Aternos!
...
async function filterProxies() {
  ...
  for (let i = 0; i < RAW_PROXIES.length; i += BATCH_SIZE) {
    ...
    if (VALID_PROXIES.length >= 50) break
  }
...
async function main() {
  ...
  if (VALID_PROXIES.length === 0) {
    console.log('❌ На этот раз живых прокси не найдено. Перезапустите скрипт через минуту.')
    return
  }
