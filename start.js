...
const SPAM_PHRASES = [
  'Всем привет!', 'Как дела народ?', 'Ищу друга', 
  'Где тут спавн?', 'Атернос лагает', 'Дайте алмазов'
]
...
function createBot(botId) {
  ...
  function start() {
    const bot = mineflayer.createBot({
      ...
      connectTimeout: 20000, viewDistance: 'tiny', physicsEnabled: false
    })
    ...
    bot.on('spawn', () => {
      if (!spamTimer) {
        spamTimer = setInterval(() => {
          ...
          console.log(`✉ Бот [${botName}] успешно зашел и отправил спам!`)
